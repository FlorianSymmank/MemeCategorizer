...
def main():
    ...
    median_color_files = [load_data(os.path.splitext(file)[
        0]) for file in data_files if "median_color" in load_data(os.path.splitext(file)[0])]
    ...
    with open(path, "w") as f:
        ...
        # f.write("255 255 255 \n")
        #endregion
        for data in median_color_files:
            f.write(" ".join(str(x) for x in data["median_color"]) +"\n")
    ...
    sub_image_width = sub_image_height = 400
    ...
    for y in range(height):
        print(y) # some progress bar ig
        for x in range(width):
            meme = find_matching_meme(pixels[x, y], pcd_tree, pcd, median_color_files)
    ...
    big_img.save('result10.png')

def find_matching_meme(coord, pcd_tree, pcd, median_color_files):
    n = 10
    ...
    color = points[idx[r]]
    roughly_equal_meme = [x for x in median_color_files if (np.array(x["median_color"]) == np.array(color)).all()]
